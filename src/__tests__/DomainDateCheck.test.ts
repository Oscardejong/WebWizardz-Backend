...
describe("DomainDateCheck", () => {
  // Helper om een mock Domain te maken met start- en einddatum
  function createMockDomain(startDate: Date, endDate: Date): Domain {
    return {
  ...
  it("should fail if start date is in the past", () => {
    ...
    const mockDomain = createMockDomain(pastDate, futureDate);
  ...
  //   const mockDomain = createMockDomain(startDate, lessThan24hLater);
  ...
  //   // Arrange
  //   const startDate = new Date(Date.now() + 1000 * 60 * 60); // over 1 uur vanaf nu
  //   const atLeast24hLater = new Date(startDate.getTime() + 1000 * 60 * 60 * 26); // 24 uur later
  //   const mockDomain = createMockDomain(startDate, atLeast24hLater);
